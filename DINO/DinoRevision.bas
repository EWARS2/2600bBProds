 rem set kernel_options pfcolors no_blank_lines background
 set kernel_options no_blank_lines
 set romsize 2k
 
 set optimization size
 rem set optimization noinlinedata
...
__physics
 if player0y >= 65 then gosub __touchingGnd else gosub __gravity
 return
...
__nextObj
 _rng=(rand&3)
 player1x=157+(_rng)
 if _rng=3 then player1y=56 else player1y=65
...
__obbird2
 ...
 rem Enables the starfield effect thing
 vblank
 asm
 sta HMCLR
 sta WSYNC
 ...
 rem To make cool line at bottom of display to be the desert floor
 asm
minikernel
 sta WSYNC
 lda $06
